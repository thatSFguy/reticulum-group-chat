...
// Runs in its own goroutine so it doesn't block the inbox handler.
func (s *Service) replayHistoryTo(joinerHash []byte, now time.Time) {
	...
	for _, e := range entries {
		...
		s.sendDirect(joinerHash, "", body)
	}
}

// humanizeAge renders a duration as "5s", "12m", "3h", "4d". Always
// rounds down; for replay context that's preferable to "in the future"
// rounding artifacts.
